...
import { View, Text, TouchableOpacity, ActivityIndicator, ScrollView, Dimensions, Image, Alert } from 'react-native'
...
const ProductDetailsPage = () => {
  ...
  return (
    <SafeScreen>
      {/** HEADER */}
      <View className='absolute right-0 left-0 top-0 justify-between flex-row px-5 py-14 z-10'>
        <TouchableOpacity
          className='bg-black/50 backdrop-blur-lg p-2 rounded-full'
          onPress={() => router.back()}
          activeOpacity={0.7}
        >
      ...
      >
          ...
          >
            {product.image.map((image: string, index: number) => (
              <View key={index} style={{ width }}>
                <Image source={{ uri: image }} style={{ width, height: 400 }} resizeMode='cover' />
              </View>
            ))}
          </ScrollView>
          {/* Image Indicators */}
       ...
       {/* Bottom Action Bar */}
      <View className="absolute bottom-8 left-0 right-0 bg-background/95 backdrop-blur-xl border-t border-surface px-6 py-4 pb-8">
